...
class RecipeDetailViewController: UIViewController, RecipeController {
    ...
    override func viewDidLoad() {
        ...
        dataStoreSubscriber = dataStore.$allRecipes
            ...
            .sink { [weak self] recipes in
                ...
                if let updatedRecipe = recipes.first(where: { $0.id == recipe.id }) {
                    self.recipe = updatedRecipe
                } else {
                    self.recipe = nil // The recipe was deleted.
                }
            }
    ...
    override func canPerformAction(_ action: Selector, withSender sender: Any?) -> Bool {
        if action == #selector(toggleFavorite(_:)) {
            #if targetEnvironment(macCatalyst)
            if let toolbarItem = sender as? NSToolbarItem {
                toolbarItem.image = favoriteImage(for: self.recipe)
            }
            #endif
        }
        
        let hasRecipe = self.recipe != nil
        if action == #selector(editRecipe(_:)) {
            return hasRecipe
        } else if action == #selector(toggleFavorite(_:)) {
            return hasRecipe
...
extension RecipeDetailViewController {
    ...
    @IBAction func toggleFavorite(_ sender: Any?) {
        ...
        #if targetEnvironment(macCatalyst)
        if let toolbarItem = sender as? NSToolbarItem {
            if toolbarItem.itemIdentifier == .toggleRecipeIsFavorite {
                toolbarItem.image = favoriteImage(for: recipe)
            }
        }
        #endif
    }
...
extension RecipeDetailViewController {
    ...
    fileprivate func updateUI() {
        ...
        bottomChildController?.recipe = recipe
        recipeFavoriteButton.image = favoriteImage(for: recipe)
        
        #if targetEnvironment(macCatalyst)
        if let touchBar = self.touchBar {
            if let item = touchBar.item(forIdentifier: .toggleRecipeIsFavorite) as? NSButtonTouchBarItem {
                item.image = recipeFavoriteButton.image
            }
        }
        #endif
    ...
    fileprivate func favoriteImage(for recipe: Recipe?) -> UIImage? {
        var imageName = "heart"
...
extension RecipeDetailViewController {
    ...
    private func configureActivityItems() {
        ...
        guard
            ...
            let toolbarDelegate = toolbar.delegate as? ToolbarDelegate
        else { return }
